...
class SpiralEncoderVAE(nn.Module):
    def __init__(self, filters_enc, latent_size, sizes, spiral_sizes, spirals, D, device, activation='elu'):
        super(SpiralEncoderVAE, self).__init__()
        ...
        for i in range(len(spiral_sizes) - 1):
            if filters_enc[1][i]:
                self.conv.append(SpiralConv(input_size, spiral_sizes[i], filters_enc[1][i], activation=self.activation, device=device))
                input_size = filters_enc[1][i]
            self.conv.append(SpiralConv(input_size, spiral_sizes[i], filters_enc[0][i+1], activation=self.activation, device=device))
            input_size = filters_enc[0][i+1]

        # Instead of a single output, we generate mean and log-variance
        self.fc_mu = nn.Linear((sizes[-1]+1) * input_size, latent_size)
        self.fc_logvar = nn.Linear((sizes[-1]+1) * input_size, latent_size)
...
class SpiralDecoder(nn.Module):
    def __init__(self, filters_dec, latent_size, sizes, spiral_sizes, spirals, U, device, activation='elu'):
        ...
        for i in range(len(spiral_sizes)-1):
            if i != len(spiral_sizes)-2:
                self.dconv.append(SpiralConv(input_size, spiral_sizes[-2-i], filters_dec[0][i+1], activation=self.activation, device=device))
                input_size = filters_dec[0][i+1]  
                if filters_dec[1][i+1]:
                    self.dconv.append(SpiralConv(input_size, spiral_sizes[-2-i], filters_dec[1][i+1], activation=self.activation, device=device))
                    input_size = filters_dec[1][i+1]
            else:
                if filters_dec[1][i+1]:
                    self.dconv.append(SpiralConv(input_size, spiral_sizes[-2-i], filters_dec[0][i+1], activation=self.activation, device=device))
                    input_size = filters_dec[0][i+1]                      
                    self.dconv.append(SpiralConv(input_size, spiral_sizes[-2-i], filters_dec[1][i+1], activation='identity', device=device)) 
                    input_size = filters_dec[1][i+1] 
                else:
                    self.dconv.append(SpiralConv(input_size, spiral_sizes[-2-i], filters_dec[0][i+1], activation='identity', device=device))
                    input_size = filters_dec[0][i+1]                      
